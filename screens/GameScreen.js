...
const renderListItem = (lstLength, itemData) => {
    return (
        <View style={styles.listItem}>
            <BodyText>#{lstLength - itemData.index}</BodyText>
...
const GameScreen = props => {
    ...
    const [avaliableDevicHeight, setAvaliableDeviceHeight] = useState(Dimensions.get('window').height);
    ...
    const nextGuessHandler = direction => {
        ...
        const nextNumber = generateRamdomBetween(currentLow.current, currentHight.current, currentGuess);
        setCurrentGuess(nextNumber);
        setPastGuesses(curPastGuesses => [nextNumber.toString(), ...curPastGuesses,])
    }

    if (avaliableDevicHeight < 500) {
        return (
            <View style={styles.screen}>
                <Text style={DefaultStyles.title}>Opponent's Guees</Text>
